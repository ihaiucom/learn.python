...
def main(argv):
	...
	copyDestDir = outputDir + "/Unable to open archive file"
	...
	for file in dirs:
		...
		if not os.path.isdir(path) and path.endswith(".txt"):
			...
			matchObj = re.match( r'\[Error\] Unable to open archive file: (.*)\n', content, re.M|re.I)
			if matchObj:
	...
	fo = open(outputDir + "/Unable to open archive file.txt", 'w')
	...
	totalSimalAll = 0
	totalSimalLine = 0
	fo = open(outputDir + "/Unable to open archive file simple.txt", 'w')
