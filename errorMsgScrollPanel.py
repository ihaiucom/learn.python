...
def main(argv):
	...
	copyDestDir = outputDir + "/MsgScrollPanel"
	...
	for file in dirs:
		...
		if not os.path.isdir(path) and path.endswith(".txt"):
			...
			matchObj = re.search( r'\[Error\] ShowByLangId MsgScrollPanel Error \: content is null at \:(.*)', content, re.M|re.I)
			if matchObj:
	...
	fo = open(outputDir + "/MsgScrollPanel.txt", 'w')
